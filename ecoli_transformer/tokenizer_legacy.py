...
class CodonTokenizer:
    ...
    def id_to_codon_safe(self, idx: int) -> str:
        """
        Convert any token ID to a 3-nucleotide codon string.
        Handles both raw codons (ATG) and AA-codon tokens (M_ATG).
        Returns empty string for special tokens or invalid IDs.
        """
        ...
        # Handle raw codon tokens (ATG, GCA, etc.)
        if len(tok) == 3 and all(c in BASES for c in tok):
            return tok
        
        # Handle AA-codon composite tokens (M_ATG, A_GCA, etc.)
        if "_" in tok:
            parts = tok.split("_")
            if len(parts) >= 2:
                maybe_codon = parts[-1]  # Take last part after underscore
                if len(maybe_codon) == 3 and all(c in BASES for c in maybe_codon):
                    return maybe_codon
...
if __name__ == '__main__':
    ...
    def id_to_codon_safe(self, idx: int) -> str:
        ...
        # Handle raw codon tokens (ATG, GCA, etc.)
        if len(tok) == 3 and all(c in "ATGC" for c in tok):
            return tok
        ...
        if "_" in tok:
            ...
            if len(parts) >= 2:
                maybe_codon = parts[-1]  # Take last part after underscore
                if len(maybe_codon) == 3 and all(c in "ATGC" for c in maybe_codon):
                    return maybe_codon
